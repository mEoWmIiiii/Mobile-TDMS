  ...
  return (
    <View style={[styles.root, { backgroundColor: colors.background }]}>
      ...
      >
        ...
        <View style={styles.section}>
          <View style={styles.sectionHeader}>
            ...
            <TouchableOpacity style={[styles.filterBtn, { borderColor: colors.border, backgroundColor: colors.card }]}>
              ...
              <Text style={[styles.filterText, { color: colors.mutedForeground }]}>Filter</Text>
            </TouchableOpacity>
          </View>
          ...
          <View style={[styles.tableCard, { backgroundColor: colors.card, borderColor: colors.border }]}>
            <View style={[styles.tableHeader, { borderBottomColor: colors.border }]}>
              {["JOB NO.", "CUSTOMER", "ROUTE", "STATUS"].map((h) => (
                <Text key={h} style={[styles.colHeader, { color: colors.mutedForeground }]}>{h}</Text>
              ))}
            </View>
            {JOBS.map((job, idx) => {
              ...
              return (
                <View key={job.id}>
                  ...
                  <Animated.View style={{ maxHeight: maxH, overflow: "hidden" }}>
                    <View style={[styles.expandedRow, { backgroundColor: colors.secondary, borderBottomColor: colors.border }]}>
        ...
        <View style={styles.section}>
          <Text style={[styles.sectionTitle, { color: colors.foreground }]}>Active Fleet Tracker</Text>
          {TRUCKS.filter((t) => t.status === "moving" || t.status === "alert").map((truck) => {
            const idx = TRUCKS.indexOf(truck);
            const progress = truckProgresses[idx];
            return (
                ...
                <View style={styles.trackerRow}>
                  ...
                  <View style={{ flex: 1 }}>
                    ...
                    <View style={{ flexDirection: "row", alignItems: "center", gap: 4, marginTop: 2 }}>
                      ...
                      <Text style={[styles.truckLoc, { color: colors.mutedForeground }]}>{truck.location}</Text>
                    </View>
                  </View>
...
const styles = StyleSheet.create({
  ...
  filterBtn: { flexDirection: "row", alignItems: "center", gap: 4, paddingHorizontal: 10, paddingVertical: 6, borderRadius: 8, borderWidth: 1 },
  filterText: { fontSize: 12, fontWeight: "500" as const },
  tableCard: { borderRadius: 14, borderWidth: 1, overflow: "hidden" },
  tableHeader: { flexDirection: "row", paddingHorizontal: 14, paddingVertical: 10, borderBottomWidth: 1, gap: 8 },
  ...
  customerName: { flex: 1, fontSize: 13, fontWeight: "600" as const },
